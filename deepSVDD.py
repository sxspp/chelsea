...
from optim.total_trainer import TotalTrainer
...
class DeepSVDD(object):
    ...
    def train(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 50,
        ...
        self.trainer = TotalTrainer(self.objective, self.R, self.c, self.nu, optimizer_name, lr=lr,
                                       n_epochs=n_epochs, lr_milestones=lr_milestones, batch_size=batch_size,
                                       weight_decay=weight_decay, device=device, n_jobs_dataloader=n_jobs_dataloader)#RDFTrain
        # Get the model
        self.net = self.trainer.train(dataset, self.net) ############# 문제 발생 #############
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        ...
        if self.trainer is None:
            self.trainer = TotalTrainer(self.objective, self.R, self.c, self.nu,
                                           device=device, n_jobs_dataloader=n_jobs_dataloader)#RDFTrain
